...
if __name__ == "__main__":
    ...
    if way.lower() == '1' or "auto":
        images_path = file_input_loop("Enter the path to the images: ")
        file_size = os.path.getsize(file)
        total_size = 0

        for i in os.listdir(images_path):
            total_size += Injector.Injector.calculate_space(os.path.join(images_path, i), bit_num)
        ...
        for img in os.listdir(images_path):
            dir_path = os.path.dirname(Jojo_in._in(os.path.join(images_path, img), bit_num, file))
        ...
        dir_path = os.path.dirname(Jojo_in._in(img, bit_num, file))
